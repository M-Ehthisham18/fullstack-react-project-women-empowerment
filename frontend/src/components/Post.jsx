...
const Post = ({ post }) => {
  ...
  const { getUser, deletePost } = usePostStore();
  ...
  const deletePostHandler = async () => {
    try {
      ...
      const response = await deletePost(_id, userId);
      if (response) {
  ...
  return (
    <div className="w-full rounded-lg shadow-none p-4 my-6 bg-white drop-shadow-[0px_0px_8px_rgba(0,0,0,0.18)]">
      ...
      <div className="flex justify-between items-center sm:px-4 ">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex items-center gap-4">
            <span className="font-semibold text-gray-900">{userPost?.fullName || "Unknown User"}</span>
            <span className="block text-sm text-gray-500">
              {(() => {
                ...
                if (hoursDiff < 24) {
                  return hoursDiff > 0 ? `${hoursDiff}hr` : `${minutesDiff}min`;
                }
                if (daysDiff < 7) {
                  return `${daysDiff}d`;
                }
        ...
        <div className="relative menu-container">
          <MoreVert className="cursor-pointer" onClick={() => setMenuOpen(!menuOpen)} />
          {menuOpen && (
            <div className="absolute right-5 top-0 mt-2 w-36 bg-white border rounded-lg shadow-lg">
              <ul className="text-left text-gray-800">
                <li className="flex px-4 py-2 hover:bg-gray-100 cursor-pointer" onClick={shareHandler}>
                  <ShareIcon className="w-4 h-4 mr-2 text-gray-700" /> Share
                </li>
                <li className="flex px-4 py-2 hover:bg-gray-100 cursor-pointer">
                  <Bookmark className="size-6 mr-2 text-gray-700 fill-none" /> Save
                </li>
                {authUser?._id === userId && (
                  <li className="flex px-4 py-2 hover:bg-red-100 text-red-500 cursor-pointer" onClick={deletePostHandler}>
      ...
      <div className="mt-4">
        <span className="text-gray-800 text-sm flex text-start sm:px-4">{desc || "No description available"}</span>
        {image && <img className="mt-4 w-full max-h-96 object-contain rounded-lg" src={image} alt="Post" />}
      </div>
